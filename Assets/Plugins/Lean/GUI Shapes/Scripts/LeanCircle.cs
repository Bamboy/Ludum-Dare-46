...
namespace Lean.Gui
{
	...
	public class LeanCircle : MaskableGraphic
	{
		...
		public override Texture mainTexture
		{
			get
			{
				if (blurTextureSet == false)
				{
					blurTextureSet = true;
					blurTexture    = new Texture2D(128, 1, TextureFormat.ARGB32, false, true);
#if UNITY_EDITOR
					blurTexture.hideFlags = HideFlags.DontSaveInEditor | HideFlags.DontSaveInBuild;
#endif
					...
					for (var i = 0; i < 128; i++)
					{
						...
						blurTexture.SetPixel(i, 0, Color.Lerp(outerColor, innerColor, snd));
					}
		...
		protected override void OnPopulateMesh(VertexHelper vertexHelper)
		{
			...
			innerVert.uv0   = Vector2.zero;

			outerVert.color = color;
			outerVert.uv0   = Vector2.one;
