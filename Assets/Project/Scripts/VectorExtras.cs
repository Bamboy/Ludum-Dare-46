...
public class VectorExtras
{
    ...
    public static Vector3 WeightedAverage( List<Vector3> values, List<float> weights )
    {
	    if( values == null || weights == null || values.Count != weights.Count )
		    return Vector3.zero;

        Vector3 valueSum = Vector3.zero;
        float weightSum = 0f;
        for( int i = 0; i < weights.Count; i++ )
        {
            valueSum += weights[i] * values[i];
            weightSum += weights[i];
        }
    ...
    /// Mirrors the value between min and max
    /// </summary>
    public static int MirrorValue( int val, int min, int max )
	{
		return min + (max - val);
	}
	...
	public static Vector2 ClampMagnitude( Vector2 vector, float minMagnitude, float maxMagnitdue )
	{
		float mag = vector.magnitude;
		if( mag > minMagnitude && mag < maxMagnitdue )
			return vector;
    ...
    public static Vector2 AnchoredMovePosTowardTarget( Vector2 origin, Vector2 direction, Vector2 maxDistance )
	{
        float mag = direction.magnitude;
        if( mag == 0f )
            return Vector2.zero;

        Vector2 intersect = direction.normalized * maxDistance.normalized;

        return intersect * maxDistance.magnitude;
	}
    ...
    /// <summary>
    /// Returns the projection of 'Vector' on to 'Normal'.
    /// </summary>
    public static Vector2 Project( Vector2 vector, Vector2 normal )
    {
        float dot = Vector2.Dot( vector, normal ) / Vector2.Dot( normal, normal );

        return new Vector2(dot * vector.x, dot * vector.y);
    }
	//===========================================
	...
	/// Returns the sign of 'val'.
	/// Output will be -1 if 'val' is negative, 0 if 0, or 1 if positive.
	public static Vector2Int Sign01( Vector2Int val )
	{
		return new Vector2Int( Sign01(val.x), Sign01(val.y) );
	}
	/// Returns the sign of 'val'.
	/// Output will be -1 if 'val' is negative, 0 if 0, or 1 if positive.
	public static Vector3Int Sign01( Vector3Int val )
	{
		return new Vector3Int( Sign01(val.x), Sign01(val.y), Sign01(val.z) );
	}
